...
export function CouponModal({ coupon, isOpen, onClose }: CouponModalProps) {
    ...
    useEffect(() => {
        if (isOpen && coupon) {
            ...
            // Don't auto-open here - handled by CouponCard
        } else {
    ...
    const handleVisitStore = () => {
        if (!coupon) return;
        const targetUrl = coupon.store_website_url || `/store/${coupon.store_slug}`;
    ...
    // Use portal to render modal at body level to avoid clipping
    return createPortal(
