        ...
        <div className="store-ui-shell store-ui-layout">
            ...
            <main className="store-ui-main">
                <section className="store-ui-coupon-section" id="store-ui-offers">
                    <div className="store-ui-coupon-toolbar"><h2>{displayName} coupons & offers</h2><div className="store-ui-filter-panel"><div className="store-ui-segments" role="group" aria-label="Offer filter">{([{ id: 'all', label: `All (${coupons.length})` }, { id: 'codes', label: `Codes (${codeCount})` }, { id: 'deals', label: `Deals (${dealCount})` }] as { id: Filter; label: string }[]).map((item) => <button type="button" className={filter === item.id ? 'active' : ''} onClick={() => setFilter(item.id)} aria-pressed={filter === item.id} key={item.id}>{item.label}</button>)}</div><label><span>Sort by:</span><select value={sort} onChange={(event) => setSort(event.target.value as Sort)}><option value="popular">Popular</option><option value="newest">Newest</option><option value="discount">Discount</option></select></label></div></div>
