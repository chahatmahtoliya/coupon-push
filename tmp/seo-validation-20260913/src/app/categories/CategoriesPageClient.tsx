...
export default function CategoriesPageClient({ initialCategories }: { initialCategories: Category[] }) {
    ...
    useEffect(() => {
        let active = true;
        categoriesApi.getAllFresh().then((fresh) => { if (active) setCategories(fresh.filter(category => getCategoryPath(category.slug) !== '/categories/')); }).catch((error) => console.error('Failed to refresh categories:', error));
        return () => { active = false; };
    }, []);
