...
const aliases = JSON.parse(await fs.readFile(new URL('../src/data/store-redirects.json', import.meta.url), 'utf8'));
...
for (const slug of Object.keys(snapshot.stores)) {
    ...
    redirects.set(`/${slug}`, `/store/${aliases[slug] || slug}/`);
}
for (const [alias, canonical] of Object.entries(aliases)) {
    redirects.set(`/store/${alias}`, `/store/${canonical}/`);
    redirects.set(`/${alias}`, `/store/${canonical}/`);
}
